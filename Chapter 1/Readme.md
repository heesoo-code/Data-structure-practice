...
2. Ω (Big-Omega)-표기법
    ...
    평균의 경우를 상정하여 평균을 찾는 분석방법이다. 버스를 예로 들면 10번의 버스를 기다렸다고 치면 걸린시간을 전부 더하고 10으로 나눠서 평균적으로 버스가 오는 시간을 구하는 방식이다. 점근적 하한만 알고 있을떄 사용하는 표기법이다.

3. Θ (Theta)-표기법

    최선의 경우를 상정하여 하한선을 두는 분석방법이다. 버스를 예로 들면 모든 변수가 긍정적으로 작용해서 빨리 도착한다면 3분만에 도착한다고 생각하는 방식이다. 상한과 하한을 둘다 알고 있을때 사용하는 표기법이다.
